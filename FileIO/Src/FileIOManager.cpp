...
namespace Fio
{
    namespace
    {
        ...
        size_t writeExtensions(const std::vector<std::string>& exts,
    ...
    bool FileIOManager::importFile(const char* filePath, FileFormat format,
        Eg::SyEntity*** outEntities, size_t* outCount,
        char* errorBuffer, size_t errorBufferSize)
    {
        return importFile(filePath, format, outEntities, outCount,
            errorBuffer, errorBufferSize, nullptr, nullptr, nullptr);
    }
        ...
        size_t* outLayerCount)
    {
        ...
        SY_INFOF("[FileIO] Importing file: %s (format=%d)",
            filePath ? filePath : "", static_cast<int>(format));
        ...
        char* errorBuffer, size_t errorBufferSize)
    {
        ...
        if (fmt == FileFormat::Unknown)
        {
            writeError(errorBuffer, errorBufferSize,
                std::string("Unsupported file format: ") + (filePath ? filePath : ""));
            return false;
        }
        return importFile(filePath, fmt, outEntities, outCount,
            errorBuffer, errorBufferSize);
    }

    bool FileIOManager::importToIR(const char* filePath, FileFormat format,
        ...
        char* errorBuffer, size_t errorBufferSize)
    {
        ...
        SY_INFOF("[FileIO] Importing to IR: %s (format=%d)",
            filePath ? filePath : "", static_cast<int>(format));
        ...
        SY_INFOF("[FileIO] Imported IR: %u entities, %u layers",
    ...
    bool FileIOManager::exportFile(const char* filePath, FileFormat format,
        const Eg::SyEntity* const* entities, size_t entityCount,
        char* errorBuffer, size_t errorBufferSize)
    {
        SY_INFOF("[FileIO] Exporting file: %s (format=%d, entities=%zu)",
            filePath ? filePath : "", static_cast<int>(format), entityCount);
        ...
        char* errorBuffer, size_t errorBufferSize)
    {
        ...
        return exportFile(filePath, fmt, entities, entityCount,
            errorBuffer, errorBufferSize);
    }
    ...
    bool FileIOManager::canImport(const char* filePath) const
    {
        ...
        return fmt != FileFormat::Unknown
            && FileParserFactory::instance().hasParser(fmt);
    }
