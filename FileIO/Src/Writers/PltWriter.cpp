    ...
    WriteResult PltWriter::write(const char* filePath, const VecSyEntityPtr& entities)
    {
        ...
        for (const auto& entity : entities)
        {
            ...
            case Eg::EType::CIRCLE:
            {
                ...
                hpgl << "PU" << toPlu(circle->basePoint.x() + circle->dRadius) << ',' << toPlu(circle->basePoint.y())
                     << ";\n";
            ...
            case Eg::EType::POLYGON:
            {
                ...
                {
                    hpgl << "PD" << toPlu(verts[i].x()) << ',' << toPlu(verts[i].y()) << ";\n";
                }
                if (polygon->bClosed)
                {
                    hpgl << "PD" << toPlu(first.x()) << ',' << toPlu(first.y()) << ";\n";
            ...
            case Eg::EType::SMARTLINE:
            {
                ...
                for (size_t si = 0; si < smartLine->segmentCount(); ++si)
                {
                    ...
                    case Eg::EType::LINE:
                    {
                        ...
                        hpgl << "PU" << toPlu(ln->pointRef().front().x()) << ',' << toPlu(ln->pointRef().front().y())
                             << ";\n";
                        for (size_t pi = 1; pi < ln->pointRef().size(); ++pi)
                        {
                            hpgl << "PD" << toPlu(ln->pointRef()[pi].x()) << ',' << toPlu(ln->pointRef()[pi].y())
                                 << ";\n";
                        }
                        break;
                    }
