...
namespace Fio
{
    namespace
    {
        std::string colorToHex(const Ut::Vec3f& color)
        {
            ...
            oss << '#' << std::hex << std::setfill('0') << std::setw(2) << toByte(color.x()) << std::setw(2)
                << toByte(color.y()) << std::setw(2) << toByte(color.z());
            return oss.str();
        }
    }  // namespace
    ...
    WriteResult SvgWriter::write(const char* filePath, const VecSyEntityPtr& entities)
    {
        ...
        for (const auto& entity : entities)
        {
            ...
            const std::string stroke = (entityColor.r() == 0.0f && entityColor.g() == 0.0f && entityColor.b() == 0.0f)
                ? "#000000"
                : colorToHex(Ut::Vec3f(entityColor.r(), entityColor.g(), entityColor.b()));
            const std::string attrs =
                "fill=\"none\" stroke=\"" + stroke + "\" stroke-width=\"2\"";  // Increased stroke width for visibility
            ...
            case Eg::EType::CIRCLE:
            {
                ...
                body << "<circle " << attrs << " cx=\"" << (circle->basePoint.x() + offsetX) << '"' << " cy=\""
                     << (offsetY - circle->basePoint.y()) << '"' << " r=\"" << circle->dRadius << "\"/>\n";
                ++exported;
                break;
            }
            case Eg::EType::ARC:
            {
                ...
                body << "<path " << attrs << " d=\"M " << (sx + offsetX) << ' ' << (offsetY - sy) << " A "
                     << arc->dRadius << ' ' << arc->dRadius << " 0 " << largeArc << ' ' << sweepFlag << ' '
            ...
            case Eg::EType::ELLIPSE:
            {
                ...
                body << "<ellipse " << attrs << " cx=\"" << (ellipse->basePoint.x() + offsetX) << '"' << " cy=\""
                     << (offsetY - ellipse->basePoint.y()) << '"' << " rx=\"" << ellipse->dRadiusX << '"' << " ry=\""
                     << ellipse->dRadiusY << '"' << " transform=\"rotate(" << (-ellipse->dRotation * 180.0 / M_PI) << ' '
                     << (ellipse->basePoint.x() + offsetX) << ' ' << (offsetY - ellipse->basePoint.y()) << ")\"/>\n";
                ++exported;
                break;
            }
            ...
            case Eg::EType::BEZIER:
            {
                ...
                body << "<path " << attrs << " d=\"M " << (bezier->basePoint.x() + offsetX) << ' '
                     << (offsetY - bezier->basePoint.y()) << " C " << (bezier->ptCtrl0.x() + offsetX) << ' '
                     << (offsetY - bezier->ptCtrl0.y()) << ' ' << (bezier->ptCtrl1.x() + offsetX) << ' '
                     << (offsetY - bezier->ptCtrl1.y()) << ' ' << (bezier->ptEnd.x() + offsetX) << ' '
            ...
            case Eg::EType::BEZIER2:
            {
                ...
                std::ostringstream d;
                d << "M " << (bezier2->basePoint.x() + offsetX) << ' ' << (offsetY - bezier2->basePoint.y());
            ...
            case Eg::EType::SMARTLINE:
            {
                ...
                for (size_t si = 0; si < smartLine->segmentCount(); ++si)
                {
                    ...
                    case Eg::EType::ARC:
                    {
                        ...
                        body << "<path " << attrs << " d=\"M " << (sx + offsetX) << ' ' << (offsetY - sy) << " A "
                             << arc->dRadius << ' ' << arc->dRadius << " 0 " << largeArc << ' ' << (sweep >= 0 ? 0 : 1)
                             << ' ' << (ex + offsetX) << ' ' << (offsetY - ey) << "\"/>\n";
                        break;
                    }
                    case Eg::EType::CIRCLE:
