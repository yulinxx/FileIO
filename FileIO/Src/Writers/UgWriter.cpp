...
namespace Fio
{
    namespace
    {
        ...
        void writeDirectoryEntry(
            std::ostream& out, int& dirSeq, int entityType, int paramPointer, int paramCount, int color = 256)
        {
            // 第1行 (DE1)
        ...
        void writeParameterLine(std::ostream& out, int entityType, const std::string& params, int& paramSeq, bool isLast)
        {
        ...
        // 添加图元到 IGES 输出
        void addEntityToIges(std::ostream& out, const Eg::SyEntity* entity, int& dirSeq, int& paramSeq)
        {
            if (!entity)
            {
                return;
            ...
            case Eg::EType::LINE:
            {
                ...
                for (size_t i = 1; i < line->pointRef().size(); ++i)
                {
                    ...
                    params << ",1,1," << igesDouble(line->pointRef()[i - 1].x()) << ","
                           << igesDouble(line->pointRef()[i - 1].y()) << "," << igesDouble(0.0) << ","
                           << igesDouble(line->pointRef()[i].x()) << "," << igesDouble(line->pointRef()[i].y()) << ","
                           << igesDouble(0.0) << ";";
            ...
            case Eg::EType::CIRCLE:
            {
                ...
                params << ",0,2,1,0,"  // DE pointer(占位), 2个子参数, 类型=圆, 关联=0
                       ...
                       << "0,"         // 平面标识
                       << igesDouble(circle->basePoint.x()) << "," << igesDouble(circle->basePoint.y()) << ","
                       << igesDouble(0.0) << "," << igesDouble(circle->dRadius) << "," << igesDouble(0.0) << ","
                       << igesDouble(2.0 * M_PI) << ";";
            ...
            case Eg::EType::ARC:
            {
                ...
                params << ",0,2,1,0,"
                       << "1,100,1,"
                       << "0," << igesDouble(arc->basePoint.x()) << "," << igesDouble(arc->basePoint.y()) << ","
                       << igesDouble(0.0) << "," << igesDouble(arc->dRadius) << "," << igesDouble(arc->dStartAngle)
            ...
            case Eg::EType::ELLIPSE:
            {
                ...
                std::ostringstream params;
                params << ",0," << igesDouble(ellipse->basePoint.x()) << "," << igesDouble(ellipse->basePoint.y())
                       << "," << igesDouble(0.0) << "," << igesDouble(ellipse->dRadiusX * std::cos(ellipse->dRotation))
                       << "," << igesDouble(ellipse->dRadiusX * std::sin(ellipse->dRotation)) << ","
                       << igesDouble(ellipse->dRadiusY / std::max(ellipse->dRadiusX, 1e-9)) << ","
                       << igesDouble(ellipse->dStartAngle) << "," << igesDouble(ellipse->dEndAngle) << ";";
            ...
            case Eg::EType::BEZIER:
            {
                ...
                std::ostringstream params;
                params << "," << kSegCount << ",0,0,1,0,0,3," << kSegCount + 1;
            ...
            case Eg::EType::SPLINE:
            {
                ...
                std::ostringstream params;
                params << "," << kSegCount << ",0,0,1,0,0," << std::max(1, spline->nDegree) << "," << kSegCount + 1;
                for (int i = 0; i <= kSegCount; ++i)
                {
                    const double t = static_cast<double>(i) / kSegCount;
        ...
        // D 段（目录段）+ P 段（参数段）需要重新生成
        {
            ...
            for (const auto& entity : entities)
            {
                ...
                case Eg::EType::CIRCLE:
                {
                    ...
                    params << ",0,2,1,0,"
                           << "1,100,1,"
                           << "0," << igesDouble(circle->basePoint.x()) << "," << igesDouble(circle->basePoint.y())
                           << "," << igesDouble(0.0) << "," << igesDouble(circle->dRadius) << "," << igesDouble(0.0)
                           << "," << igesDouble(2.0 * M_PI) << ";";
                ...
                case Eg::EType::POLYGON:
                {
                    ...
                    std::ostringstream params;
                    params << "," << (polygon->bClosed ? 1 : 0) << "," << verts.size();
                ...
                case Eg::EType::BEZIER2:
                {
                    ...
                    for (int i = 0; i <= kSegCount; ++i)
                    {
                        const double t = static_cast<double>(i) / kSegCount;
                        const double u = 1.0 - t;
                        const double x = u * u * bezier2->basePoint.x() + 2.0 * u * t * bezier2->ptCtrl.x() +
                            t * t * bezier2->ptEnd.x();
                        const double y = u * u * bezier2->basePoint.y() + 2.0 * u * t * bezier2->ptCtrl.y() +
                            t * t * bezier2->ptEnd.y();
                        params << "," << igesDouble(x) << "," << igesDouble(y);
                    }
                    params << ";";
                ...
                case Eg::EType::SMARTLINE:
                {
                    ...
                    for (size_t si = 0; si < smartLine->segmentCount(); ++si)
                    {
                        ...
                        case Eg::EType::LINE:
                        {
                            ...
                            {
                                std::ostringstream params;
                                params << ",1,1," << igesDouble(ln->pointRef()[pi - 1].x()) << ","
                                       << igesDouble(ln->pointRef()[pi - 1].y()) << "," << igesDouble(0.0) << ","
                                       << igesDouble(ln->pointRef()[pi].x()) << ","
                                       << igesDouble(ln->pointRef()[pi].y()) << "," << igesDouble(0.0) << ";";
                                writeDirectoryEntry(dirStream, dSeq, 116, pSeq + 1, 1);
                                writeParameterLine(paramOut, 116, params.str(), pSeq, true);
                            }
                            break;
                        ...
                        case Eg::EType::ARC:
                        {
                            ...
                            params << ",0,2,1,0,"
                                   << "1,100,1,"
                                   << "0," << igesDouble(arc->basePoint.x()) << "," << igesDouble(arc->basePoint.y())
                                   << "," << igesDouble(0.0) << "," << igesDouble(arc->dRadius) << ","
                                   << igesDouble(arc->dStartAngle) << "," << igesDouble(arc->dEndAngle) << ";";
                            writeDirectoryEntry(dirStream, dSeq, 102, pSeq + 1, 1);
                            writeParameterLine(paramOut, 102, params.str(), pSeq, true);
                            break;
                        }
                        case Eg::EType::CIRCLE:
                        {
                            ...
                            params << ",0,2,1,0,"
                                   << "1,100,1,"
                                   << "0," << igesDouble(circle->basePoint.x()) << ","
                                   << igesDouble(circle->basePoint.y()) << "," << igesDouble(0.0) << ","
                                   << igesDouble(circle->dRadius) << "," << igesDouble(0.0) << ","
                                   << igesDouble(2.0 * M_PI) << ";";
                            writeDirectoryEntry(dirStream, dSeq, 102, pSeq + 1, 1);
                            writeParameterLine(paramOut, 102, params.str(), pSeq, true);
                            break;
